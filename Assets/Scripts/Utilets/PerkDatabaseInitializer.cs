...
namespace Sinbinder.Utilets
{
    ...
    {
        /// <summary>
        /// Только Resources: <c>BehaviourResolver</c> берёт базу через
        /// <c>Resources.Load&lt;PerkDatabase&gt;("PerkDatabase")</c>. Прежний путь
        /// вёл в папку `Assets/_Project/...`, которой в проекте нет, — ассет
        /// не создавался вовсе, а если бы создался, резолвер его не нашёл бы.
        /// </summary>
        private const string DATABASE_PATH = "Assets/Resources/PerkDatabase.asset";

        [MenuItem("Sinbinder/Собрать базу перков")]
        public static void Initialize()
        {
            if (!AssetDatabase.IsValidFolder("Assets/Resources"))
                AssetDatabase.CreateFolder("Assets", "Resources");

            // Пересобираем начисто. Перки лежат подобъектами внутри файла базы,
            // и дописывание к старому оставило бы сирот: список бы обновился,
            // а сами перки остались лежать в ассете прошлым слоем.
            if (AssetDatabase.LoadAssetAtPath<PerkDatabase>(DATABASE_PATH) != null)
                AssetDatabase.DeleteAsset(DATABASE_PATH);

            var database = ScriptableObject.CreateInstance<PerkDatabase>();
            AssetDatabase.CreateAsset(database, DATABASE_PATH);

            database.AllPerks = CreateAllPerks();

            // Без AddObjectToAsset у перка нет файла. Список сохранился бы
            // сотней пустых ссылок, PerkResolver не нашёл бы ни одного,
            // и ошибки при этом не было бы ни одной.
            for (int i = 0; i < database.AllPerks.Count; i++)
            {
                var perk = database.AllPerks[i];
                // Имена перков повторяются (Гончар, Пекарь и ещё десяток),
                // а подобъекты различают по имени. Номер — служебный,
                // игрок видит PerkName, а не имя ассета.
                perk.name = $"{i + 1:D3} {perk.PerkName}";
                AssetDatabase.AddObjectToAsset(perk, database);
            }

            EditorUtility.SetDirty(database);
            AssetDatabase.SaveAssets();
            AssetDatabase.ImportAsset(DATABASE_PATH);

            Debug.Log($"[Перки] Собрано {database.AllPerks.Count} перков: {DATABASE_PATH}");
        }
