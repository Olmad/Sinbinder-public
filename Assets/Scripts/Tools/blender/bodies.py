...
def build_human(b, p):
    ...
    stack = [(hip - 0.045, 0.092), (hip + 0.020, 0.098), (p["waist"], 0.094),
             (p["chest"], 0.104), (sh - 0.010, 0.100), (p["neck"] + 0.004, 0.070)]
    for (z0, r0), (z1, r1) in zip(stack, stack[1:]):
        b.add(*tube((0, 0.004, z0), (0, 0.004, z1), r0, r1, segs=16), bone="Spine")

    # Пояс: тёмная полоса — единственное, что делит фигуру сверху надвое.
    b.add(*ring((0, 0.004, hip + 0.012), 0.098, 0.014, scale=(1.0, 0.9, 1.0), segs=20),
          bone="Spine", mat=2)

    # Шея и голова. Голова держит крупный план наезда, поэтому у неё
    # есть глазницы-тени и нос, но нет лица: безымянные люди.
    b.add(*tube((0, 0.010, p["neck"]), (0, 0.012, p["skull"] - 0.060), 0.034, 0.032),
          bone="Neck", mat=1)
    b.add(*sphere((0, 0.010, p["skull"]), 0.072, scale=(0.96, 1.08, 1.12),
    ...
    for side, _ in sides():
        b.add(*box((side * 0.028, -0.072, p["skull"] + 0.008), (0.032, 0.014, 0.011)),
              bone="Head", mat=2)
    b.add(*box((0, -0.080, p["skull"] - 0.012), (0.017, 0.020, 0.036)), bone="Head", mat=1)

    for side, tag in sides():
        # Плечо — часть рукава, чтобы не зиять щелью у ворота.
        b.add(*sphere((side * (sx - 0.008), 0.004, sh - 0.004), 0.046,
                      scale=(1.0, 0.9, 0.85), segs=12, rings=8), bone=tag + "Shoulder")

        limb(b, tag + "UpperArm", (side * sx, 0, sh), (side * el, 0, sh),
             0.036, 0.030, joint=0.037)
        limb(b, tag + "LowerArm", (side * el, 0, sh), (side * (wr - 0.004), 0, sh),
             0.030, 0.026, joint=0.031)
        ...
        hand(b, tag + "Hand", side, wr, fg, sh, thick=0.024, mat=2)
