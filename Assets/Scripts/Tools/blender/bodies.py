...
# Пропорции в долях роста. Проверены 20 сентября замером чужих моделей
# (`reference.py`) и каноном фигуры — до того они были выбраны на глаз,
# и это было видно: голова занимала пятую часть роста вместо восьмой,
# плечи были уже бёдер, а талия не отличалась от груди.
#
# Главное расхождение с образцами: шея. У десяти скачанных моделей она
# в среднем на 0,854 роста, у человека канонически 0,86 — у нас было
# 0,795. Ровно эта разница и делала из бойца куклу.
#
# Числа не скопированы, а сведены: у образцов свой стиль (доспехи,
# рога, плащи), и брать их ширину значило бы делать чужую игру.
BASE = dict(
...
def sphere(center, r, scale=(1, 1, 1), segs=20, rings=12, holes=(), only=()):
    ...
    **Намотка граней — наружу, и это не вкусовщина.** До 13 сентября
    здесь было `[a, b, c, d]`, и все до одной грани смотрели внутрь.
    В Блендере это было не видно: Workbench рисует грань с обеих
    сторон, и превью выходили целыми. Unity заднюю грань отбрасывает —
    и игрок смотрел сквозь лоб в затылок изнутри. Череп был
    единственной сферой на всём скелете, поэтому и мерещился
    он один.

    Чтобы это не повторилось, `audit_primitives` ниже проверяет
    намотку при каждой сборке.
    """
    cx, cy, cz = center
    dirs = []
    verts = []
...
def eyes(b, centre, r, scale, dirs, size):
    """
    Огоньки в глазницах. Материал 3 — «Глаз», и он у всех четверых
    оболочек лежит под одним и тем же номером: цвет ставит игра
    по греху души (<c>SinEyes</c>), а не скрипт.

    Шарик сидит **внутри** черепа, на семи десятых радиуса: снаружи
    его не видно вовсе, видно только сквозь дыру глазницы. Тогда
    свет читается как идущий изнутри, а не как накладка на кости.

    Решение автора от 15 сентября: «у всех воинов горят глаза под цвет
    греха». Под камерой в 84 градуса голова — самое видное, а цвет
    узнаётся мгновенно и без единой цифры.
    """
    cx, cy, cz = centre
    for d in dirs:
        n = Vector(d)
        n.normalize()
        b.add(*sphere((cx + n[0] * r * scale[0] * 0.70,
                       cy + n[1] * r * scale[1] * 0.70,
                       cz + n[2] * r * scale[2] * 0.70),
                      size, segs=10, rings=6), bone="Head", mat=3)
...
def skull(b, p, r, socket, jaw, mat=0, hair=False):
    ...
    eyes(b, (0, 0.014, z), r, (1.0, 1.18, 1.06),
         [(0.46, -0.86, 0.04), (-0.46, -0.86, 0.04)], r * 0.26)
...
def foot(b, tag, side, x, mat=0, wide=1.0):
    # Стопа доходит до щиколотки. Прежняя кончалась на 0,040 при
    # щиколотке 0,055, и между голенью и сапогом зияли полтора
    # сантиметра пустоты — видно на любом превью в три четверти.
    b.add(*box((side * x, -0.030, 0.029), (0.048 * wide, 0.090, 0.054)),
...
def build_ghost(b, p):
    ...
    # У призрака глазницы вместо лица, и огонь в них крупнее всех:
    # он и есть всё, что от лица осталось.
    eyes(b, (0, 0.014, p["skull"]), 0.072, (1.0, 1.14, 1.12),
         [(0.40, -0.90, 0.02), (-0.40, -0.90, 0.02)], 0.026)
...
def build_golem(b, p):
    ...
    b.add(*box((0, 0.004, p["chest"] + 0.028), (0.265, 0.175, 0.140)), bone="Spine")
    b.add(*box((0, 0.010, p["shoulder"]), (0.372, 0.168, 0.050)), bone="Spine")
    ...
    for side, _ in sides():
        # Эти две плиты и были глазами голема; теперь они горят.
    ...
    b.add(*box((0.045, -0.090, p["chest"] + 0.038), (0.014, 0.014, 0.110)),
          bone="Spine", mat=2)
    b.add(*box((-0.090, -0.082, p["chest"] + 0.012), (0.012, 0.014, 0.062)),
          bone="Spine", mat=2)
...
def build_human(b, p):
    """
    Живое тело: охотники и сам Греховод (`ShellType.Living`).

    Слово автора от 13 сентября: «охотники — люди». Тело нарочно
    безличное — одежда без знаков, голова без черт, кроме тени глазниц
    и носа: всё, что отличает охотника от инквизитора, надевается
    сверху, частями гардероба (`wear.py`). Одно тело, много обликов —
    тот же расчёт, что в `22-LOOK.md`: двенадцать предметов вместо
    восьмидесяти одной модели.

    Арматура та же, что у оболочек, и пропорции те же, человеческие:
    иначе гардероб, сшитый по скелету, не сядет на человека, а движения
    пришлось бы писать дважды.

    Материалы — в том же порядке смысла: 0 одежда, 1 кожа, 2 тёмное.
    """
    hip, knee, ankle = p["hip"], p["knee"], p["ankle"]
    sh, sx, el, wr, fg = (p["shoulder"], p["shoulder_x"],
                          p["elbow"], p["wrist"], p["finger"])
    ...
    # Туловище — одним куском на одной кости, как у зомби и голема:
    # жёсткие веса рвут сплошную поверхность на границе костей.
    # Талия у`же груди и таза — единственное, что отличает фигуру
    # от трубы. До 20 сентября разница между ними была шесть тысячных
    # роста, то есть её не было вовсе.
    stack = [(hip - 0.052, 0.104), (hip + 0.018, 0.100), (p["waist"], 0.086),
             (p["chest"], 0.118), (sh - 0.012, 0.112), (p["neck"] + 0.006, 0.062)]
    for (z0, r0), (z1, r1) in zip(stack, stack[1:]):
        b.add(*tube((0, 0.004, z0), (0, 0.004, z1), r0, r1, segs=16), bone="Spine")

    # Пояс: тёмная полоса — единственное, что делит фигуру сверху надвое.
    b.add(*ring((0, 0.004, p["waist"] - 0.010), 0.090, 0.015, scale=(1.0, 0.9, 1.0),
                segs=20), bone="Spine", mat=2)

    # Полы куртки ниже пояса: короткий раструб поверх бёдер. Без него
    # человек стоит в обтяжку и читается голым манекеном в сапогах.
    b.add(*tube((0, 0.004, hip + 0.026), (0, 0.004, hip - 0.055), 0.101, 0.110, segs=18),
          bone="Spine")
    b.add(*ring((0, 0.004, hip - 0.053), 0.110, 0.009, scale=(1.0, 0.95, 1.0), segs=18),
          bone="Spine", mat=2)

    # Шея и голова. Голова держит крупный план наезда, поэтому у неё
    # есть глазницы-тени и нос, но нет лица: безымянные люди.
    # Ворот: тёмное кольцо у основания шеи. Оно отделяет голову
    # от плеч — без него они сливаются в один ком на дальнем плане.
    b.add(*ring((0, 0.008, p["neck"] + 0.004), 0.052, 0.016, scale=(1.0, 1.0, 0.8),
                segs=16), bone="Neck", mat=2)

    b.add(*tube((0, 0.010, p["neck"]), (0, 0.012, p["skull"] - 0.052), 0.033, 0.030),
          bone="Neck", mat=1)

    # Голова меньше прежней: при шее на 0,848 прежний радиус давал
    # шестую часть роста — голову ребёнка на теле взрослого.
    b.add(*sphere((0, 0.010, p["skull"]), 0.062, scale=(0.95, 1.06, 1.12),
                  segs=28, rings=18), bone="Head", mat=1)
    # Глаза — тень, а не глаз: две тёмные щели на поверхности черепа.
    # Ставятся снаружи головы, а не внутри: радиус 0.072 при вытяжении
    # 1.08 по глубине — это 0.078, и первая версия с −0.064 оказалась
    # под кожей, откуда её видно не было.
    # Черты считаются от поверхности черепа, а не ставятся на глаз.
    # Крупный план 20 сентября показал, зачем: глазницы выступали
    # на тринадцать миллиметров, нос на двадцать два — получался
    # не человек, а водолазный шлем с клювом.
    #
    # Поверхность на высоте h от середины головы:
    #     y = 0.010 - 0.0657 * sqrt(1 - (h / 0.0694)^2)
    # Щель глаза сидит на ней (выступ около миллиметра), нос выходит
    # вперёд на сантиметр, челюсть — на девять миллиметров.
    for side, _ in sides():
        b.add(*box((side * 0.022, -0.0528, p["skull"] + 0.006), (0.019, 0.004, 0.006)),
              bone="Head", mat=3)

    # Надбровья нет нарочно. Полка в два миллиметра на светлой голове
    # ловит верхней гранью весь свет и читается козырьком шлема —
    # проверено крупным планом. Тень над глазами даёт сама щель.

    # Нос короткий: выступ в полсантиметра. Прежний торчал на два
    # с лишним и превращал лицо в клюв.
    b.add(*sphere((0, -0.0505, p["skull"] - 0.013), 0.016,
                  scale=(0.62, 0.72, 1.35), segs=10, rings=7), bone="Head", mat=1)

    # Челюсть: она почти вся внутри черепа и выступает только подбородком.
    # Отдельный шар снаружи читался рылом, а не челюстью.
    b.add(*sphere((0, -0.004, p["skull"] - 0.040), 0.046,
                  scale=(0.98, 1.06, 0.72), segs=16, rings=10), bone="Head", mat=1)

    for side, tag in sides():
        # Плечо — часть рукава, чтобы не зиять щелью у ворота.
        # Дельта: у скелета плечо — кость, у человека ещё и мышца.
        b.add(*sphere((side * (sx - 0.010), 0.004, sh - 0.006), 0.055,
                      scale=(1.0, 0.92, 0.88), segs=12, rings=8), bone=tag + "Shoulder")

        limb(b, tag + "UpperArm", (side * sx, 0, sh), (side * el, 0, sh),
             0.042, 0.033, joint=0.043)
        limb(b, tag + "LowerArm", (side * el, 0, sh), (side * (wr - 0.004), 0, sh),
             0.033, 0.028, joint=0.034)
        # Обшлаг перчатки — тёмный, чтобы рука не сливалась с рукавом.
        b.add(*tube((side * (wr - 0.030), 0, sh), (side * (wr + 0.002), 0, sh),
                    0.031, 0.031, segs=10), bone=tag + "LowerArm", mat=2)
...
def signed_volume(verts, faces):
    """
    Объём со знаком по теореме о дивергенции. Для замкнутой поверхности
    он положителен, когда грани намотаны наружу, и отрицателен, когда
    внутрь, — и это верно для любой формы, хоть шара, хоть бублика,
    в отличие от проверки «нормаль против направления от центра»,
    которая на бублике врёт.
    """
    total = 0.0
    for f in faces:
        for k in range(1, len(f) - 1):
            a, b, c = (Vector(verts[f[0]]), Vector(verts[f[k]]),
                       Vector(verts[f[k + 1]]))
...
def audit_primitives():
    """
    Все примитивы намотаны наружу — проверяется при каждой сборке.

    Заведено 13 сентября, после того как у сферы все до одной грани
    смотрели внутрь, а заметить это в Блендере было нельзя: Workbench
    рисует грань с обеих сторон, и превью выходили целыми. Увидел
    автор — в Unity, где задняя грань отбрасывается, и череп скелета
    просвечивал насквозь.

    Мораль не про сферу, а про инструмент: **превью в Блендере не
    отвечает на вопрос, что покажет Unity.** Раз глазами не поймать,
    ловим числом.
    """
    checks = (
        ("box", box((0, 0, 0), (1, 1, 1))),
        ("sphere", sphere((0, 0, 0), 1.0)),
        ("tube", tube((0, 0, -1), (0, 0, 1), 0.3, 0.3)),
        ("ring", ring((0, 0, 0), 0.5, 0.12)),
    )
    bad = [name for name, (v, f) in checks if signed_volume(v, f) <= 0.0]
    if bad:
        raise SystemExit(
            "[ТЕЛА] Намотка внутрь: " + ", ".join(bad)
          + ". В Unity такая грань не рисуется вовсе — модель просвечивает. "
            "Переверни порядок вершин в гранях этих примитивов.")
    print("[ТЕЛА] Намотка: " + ", ".join(n for n, _ in checks) + " — наружу.")
...
def make_actions(arm, g):
    ...
    # --- Attack: удар. Оружия в руках нет — это слот класса, а не тела
    #     (23-PROMPTS.md §2), — поэтому бьёт сама рука: замах назад,
    #     хлёсткий бросок вперёд с одновременным наклоном корпуса,
    #     короткий откат. Не зациклен, как Die: воин ударил один раз,
    #     а не дёргается на месте, пока решение не сменится (BodyImport,
    #     Looping). Последний кадр — поза Idle: если решение задержится
    #     на Attack дольше клипа, воин застынет в покое, а не в перекруте.
    attack = [
        (1,  s((-6, 10, -2, 8, 6, 2), (60, 4, 8, 20), (-4, 2, 3, 0), 0.0)),
        (7,  s((-14, 20, -4, 18, 10, 4), (66, 10, 74, 82), (-18, 8, 10, 0), 0.014)),
        (12, s((14, 6, 4, -10, 24, -8), (54, -6, -58, 14), (20, -8, -12, 0), -0.024)),
        (18, s((3, 9, 0, 0, 11, 1), (66, 2, -6, 22), (5, -1, -3, 0), 0.0)),
        (24, s((-2, 4, -1, 2, 4, 1), (78, 1, -1, 12), (2, 1, 1, 0), 0.0)),
    ]

    # --- Talk: реплика. Зациклен: разговор стоит кадрами, сколько нужно
    #     реплике, а не фиксированное время. Ноги молчат совсем — говорящий
    #     не переминается, — говорит рука (поднята и слегка взмахивает)
    #     и голова (кивок), кадр 1 и последний — одна и та же поза,
    #     иначе цикл дёргается на стыке.
    talk = [
        (1,  s((-2, 4, -1, 2, 4, 1), (70, 2, -24, 34), (2, 4, 3, 6), 0.0)),
        (14, s((-2, 4, -1, 2, 4, 1), (66, -4, -34, 44), (4, 6, 4, -6), -0.004)),
        (28, s((-2, 4, -1, 2, 4, 1), (72, 4, -18, 28), (1, 3, 2, 8), 0.004)),
        (40, s((-2, 4, -1, 2, 4, 1), (70, 2, -24, 34), (2, 4, 3, 6), 0.0)),
    ]
...
# (SinEyes), и ставит каждому воину свой. Тускло-белый выбран нарочно,
# чтобы модель без игры не врала про грех, которого у неё нет.
EYE = (0.62, 0.60, 0.58, 1.0)
...
SHELLS = [
    ...
    Shell(
        # Имя файла — по имени оболочки: WarriorLook ищет Bodies/Living.
        name="Living",
        # Живой шире в плечах и длиннее в руке, чем костяк: у скелета
        # плечо — это кость, у человека ещё и мышца. До 20 сентября
        # человек делил со скелетом весь набор чисел целиком и отличался
        # от него только одеждой — это и было видно на снимках.
        parts=proportions(shoulder_x=0.128, elbow=0.302, wrist=0.448,
                          finger=0.548),
        # Одежда — потёртая тёмная кожа и сукно, кожа — бледная и тусклая:
        # ночь, холод, никакого румянца (23-PROMPTS.md §2).
        # Четвёртый слот — «Глаз». Его не было, и у живого тела глаза
        # гореть не могли в принципе: SinEyes ищет материал по имени
        # и у человека не находил ничего. А Греховод — человек, и он
        # тьма с глазами под капюшоном (слово автора от 20 сентября).
        materials=[("Cloth", (0.235, 0.205, 0.175, 1.0)),
                   ("Skin", (0.435, 0.378, 0.335, 1.0)),
                   ("Leather Dark", (0.090, 0.075, 0.065, 1.0)),
                   ("Eye", EYE)],
        build=build_human,
        gait=STRIDE,
    ),
...
def main():
    ...
    # До сборки, а не после: вывернутая грань портит все четыре разом,
    # и узнать об этом из превью нельзя.
    audit_primitives()
...
# Сборка идёт только при прямом запуске.
#
# До 17 сентября `main()` стоял здесь голым и выполнялся **при импорте**.
# А импортировать этот файл есть зачем: из него берут SHELLS, wipe,
# build_armature и build_mesh, когда собирают отдельную сцену — скажем,
# силуэты для трейлера. Каждый такой импорт молча пересобирал все четыре
# оболочки и перезаписывал FBX, и в `git status` появлялись четыре
# изменённых файла, в которых не изменилось ничего, кроме времени
# создания и внутренних номеров (см. оговорку о воспроизводимости выше).
#
# Поймано хуком, который не дал закоммитить полтора мегабайта шума.
