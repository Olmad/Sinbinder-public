...
namespace Sinbinder.EditorTools
{
    ...
    ///
    /// Модель приходит из Блендера (<c>Tools/blender/bodies.py</c>), и между
    ...
    public class BodyImport : AssetPostprocessor
    {
        ...
        private static void Wire(string modelPath)
        {
            ...
            // Покой вместо удара или разговора — терпимая деградация:
            // воин стоит, и это читается как «ещё не нарисовано».
            // Покой вместо падения — нет. Damageable.Die() тело из сцены
            // не убирает, и труп встаёт по стойке смирно среди живых:
            // обучение жатве в доле 4 и гибель Каргана в доле 5 ломаются
            // целиком (разбор коллеги, 13 сентября). Поэтому об этом
            // говорим отдельно и громче.
            if (missing.Contains(BodyMotion.Die))
                Debug.LogWarning($"[ТЕЛО] {name}: нет клипа {BodyMotion.Die} — убитый "
                               + "останется стоять. Это не «анимации пока нет», "
                               + "а поломка: доли 4 и 5 на ней держатся.");
        }
    }
}
