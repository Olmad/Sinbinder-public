...
namespace Sinbinder.Gameplay
{
    ...
    public static class BodyWorth
    {
        ...
        // на глаз. До 14 сентября все трупы пролога были зомби
        // (HunterSquadSpawner), и средняя добыча за труп обязана была
        // остаться прежней, 12 золота. Теперь охотники живые, и калибровка
        // держится на LivingBase ниже; зомби сохранил своё число. Иначе важность деяния (золото/10) поплывёт,
        ...
        // Живые — те самые охотники пролога, которые до 14 сентября ходили
        // в зомби. Калибровка средней добычи (12 золота за труп) переехала
        // сюда вместе с ними: трупы, которые игрок разбирает в прологе,
        // теперь живые, а не зомби.
        //
        // 9.8, а не прежние 10.4: вместе с переездом Мясник (Чревоугодие,
        // «всё сожрал при жизни») стал Инквизитором (Гордыня, держит при
        // себе), и при 10.4 средняя уехала на +6,3% — титулы пришли бы
        // на два трупа раньше. Замер: Tools/bench → ДОБЫЧА.
        private const float LivingBase   = 9.8f;
        ...
        private static float ShellBase(ShellType shell)
        {
            switch (shell)
            {
                ...
                case ShellType.Living:   return LivingBase;
