...
namespace Sinbinder.Gameplay
{
    ...
    public class PrologueCampSpawner : MonoBehaviour
    {
        ...
        ///
        /// <b>Тела</b> — четверо скелетов, трое зомби, двое призраков (слово
        /// автора, 25 сентября: «очень сильно не хватает разнообразия, все
        /// скелеты»). Правило раздачи — от прогона, а не от вкуса (14-HANDOFF
        /// §108): <b>в разгроме не должно оказаться зомби и призрака вместе</b>.
        /// Вместе они затягивали первую волну вдвое, и Греховод гибнул у края;
        /// порознь — нет. В разгроме остаются Карган, Ждан и двое кандидатов,
        /// которых не выбрали старшим, — поэтому призраки только среди тех, кто
        /// уходит с вылазкой, а из кандидатов новое тело лишь у Марги.
        /// Тело ещё и тянет душу при каждом появлении (<see cref="ShellBinder"/>):
        /// зомби отнимает Гордыню — гордых зомби нет, на Гордыне Каргана держится
        /// пророчество доли 6; призрак отнимает Жадность — Марга не призрак,
        /// на Жадности держится его долг.
        /// </summary>
        private static readonly CampMember[] Squad =
        {
            ...
            // Трое опытных. Грехи — три греха демо (решение автора
            // 24 сентября: Гордыня, Уныние, Жадность): Вейн — Уныние,
            // Марга — Жадность, Хальд — Гордыня. Раньше Хальд был гневным,
            // а в отряде ходили все семь грехов — демо обещает три
            // (00-GDD.md §8), и девять человек с семью грехами читались
            ...
            // Двое братьев по оружию. Хорь жаден до чужого, Гурт не говорит
        ...
        void OnDestroy()
        {
            ...
            // Сцену закрывает загрузка или «С начала доли»: состав уже
            // возвращён из записи (SaveSystem.ReturnTo — сперва состояние,
            // потом сцена). Снять его с живых уходящей сцены значило бы
            // затереть запись тем, что было в миг смерти: павшие остались бы
            // мёртвыми, ушедшие — выброшенными (14-HANDOFF §105.7).
        ...
        private void SpawnPlayer()
        {
            ...
            {
                // Жалуемся только в лагере. В склепе палатки нет по замыслу,
                // и Греховод у костра там верен; предупреждение на каждом
                // входе в склеп учило бы пропускать его и в лагере, где оно
                // означает настоящую поломку (найдено прогоном DemoSmoke).
                if (gameObject.scene.name.Contains("Camp"))
