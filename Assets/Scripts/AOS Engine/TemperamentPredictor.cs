...
namespace Sinbinder.AOS
{
    ...
    public static class TemperamentPredictor
    {
        ...
        ///
        /// Ни одна ступень не обещает твёрдо: измерено, что точность
        /// предсказания упирается в две трети. Панель, обещающая больше,
        /// чем сбывается, разрушает то самое доверие, ради которого
        /// написана.
        /// </summary>
        private static string Describe(Decision decision, DecisionContext context)
        {
            ...
            // Приказ, проигравший голосование, обязан читаться как отказ.
            // «Пойдёт в драку» — правда, но не та правда: игрок должен
            // прочитать не что воин сделает, а чего он НЕ сделает из
            // велено́го. На этой строке держится третье требование к демо
            ...
            //
            // Стенд (Tools/bench) прогнал пророчества против похожих,
            // но не тождественных положений: тот же определяющий признак,
            // остальное случайно. Сбывается 50–65%, и — главное —
            // точность упирается в потолок около двух третей и выше
            // не растёт. При самой высокой уверенности она даже падает.
            //
            // Причина не в движке, а в природе обещания: пророчество
            // говорит о положении («когда падает друг»), а поступок
            // зависит ещё и от здоровья, усталости, окружения и добычи
            // под ногами. Внутри одного положения этого разброса хватает,
            // чтобы никакая уверенность не давала твёрдого слова.
            //
            // Значит, порога для прямого обещания не существует, и
            // прежняя формулировка без оговорки («держит строй») врала
        ...
        private static DecisionContext OrderedBack(Warrior warrior)
        {
            var c = Base(warrior);
            c.HasCommand = true;
            c.CommandType = CommandKind.FallBack.ToString();
            c.CommandIsFallBack = true;
            c.CommandLeavesFight = true;
            c.IsEngaged = true;
            c.EngagedWith = 1;
            return c;
        }
