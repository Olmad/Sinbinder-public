...
namespace Sinbinder.AOS
{
    ...
    public static class TemperamentPredictor
    {
        ...
        private static string Describe(Decision decision, DecisionContext context)
        {
            ...
            // Втрое увереннее порога — говорим прямо, без оговорки.
            // Мера та же, что у резолвера: пророчество и движок обязаны
            // мерить одним, иначе панель пообещает не то, что случится.
            if (decision.Confidence > BehaviourResolver.HesitationShare * 3f) return what;
