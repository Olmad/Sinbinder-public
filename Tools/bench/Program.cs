...
static class Bench
{
    ...
    static void CampUnderOrderCheck(AOSConfig cfg)
    {
                        ...
                        float Intensity, float Loyalty, int Unpaid)[]
        {
            ...
            ("Хальд",   SinType.Pride,    MoralType.Pious,   35f, 95f, 0),
            ("Хорь",    SinType.Greed,    MoralType.Vicious, 45f, 65f, 0),
            ("Ю",       SinType.Sloth,    MoralType.Neutral, 25f, 80f, 0),
            ("Лиска",   SinType.Greed,    MoralType.Neutral, 30f, 85f, 0),
    ...
    static void VoiceCheck(AOSConfig cfg)
    {
        ...
        var squad = new (string Name, SinType Sin, MoralType Moral, float Intensity, float Loyalty)[]
        {
            ...
            ("Хальд",  SinType.Pride,    MoralType.Pious,   35f, 95f),
            ("Хорь",   SinType.Greed,    MoralType.Vicious, 45f, 65f),
            ("Ю",      SinType.Sloth,    MoralType.Neutral, 25f, 80f),
            ("Лиска",  SinType.Greed,    MoralType.Neutral, 30f, 85f),
    ...
    /// <summary>
    /// Как зовут по заслугам мужчину и как женщину.
    ///
    /// До 17 сентября пол титул не спрашивал, и в базе стояла
    /// <b>«Защитница»</b> самостоятельным правилом на двенадцать
    /// спасений: мужчина с двенадцатью спасениями становился
    /// «Защитница Марга». Теперь у правила одно условие и две формы
    /// слова, а пара пишется руками — правило по окончанию здесь
    /// не работает («Беглец» даёт «Беглянка», «Скупой» — «Скупая»).
    ///
    /// Сторожим ровно ту беду, что была: <b>ни одно слово, помеченное
    /// женским, не имеет права быть чьей-то мужской формой</b>.
    /// Правилом это не вывести, списком — выводится.
    /// </summary>
    static void TitleGenderCheck()
    {
        Console.WriteLine("\n=== ТИТУЛЫ: одно условие, две формы ===");
        Console.WriteLine("  деянию всё равно, чьё это тело, — слову нет\n");
        Console.WriteLine($"  {"мужчине",-24} {"женщине",-24} порог");
        ...
        int pairs = 0;
        var seenHe = new HashSet<string>();
        var seenShe = new HashSet<string>();

        foreach (var r in TitleDatabase.Rules)
        {
            // Главное требование, и оно же единственное, которое здесь
            // вообще можно проверить: обе формы объявлены. Из слова
            // «Защитница» не вывести, что оно женское, а из «Тень» —
            // что оно общее; правило по окончанию тут врёт на обоих.
            // Зато «поле не заполнено» видно всегда, и заполнить его
            // значит решить вопрос, а не обойти его.
            Check(!string.IsNullOrEmpty(r.Female),
                  $"{r.Title}: женская форма не объявлена — как её носит женщина, никто не решил");

            string he = r.For(Gender.Male);
            string she = r.For(Gender.Female);

            Check(he == r.Title, $"{r.Title}: мужская форма — это само слово");
            Check(seenHe.Add(he), $"{he}: слово мужчине занято дважды");
            Check(seenShe.Add(she), $"{she}: слово женщине занято дважды");

            if (he == she) continue;

            pairs++;
            Console.WriteLine($"  {he,-24} {she,-24} {r.RequiredCount}");
        }

        Console.WriteLine();
        Console.WriteLine($"  форма своя: {pairs}, "
                        + $"слово одно на обоих: {TitleDatabase.Rules.Count - pairs}");
        Console.WriteLine(bad == 0
            ? "  У всех двадцати восьми решено, как имя носит женщина."
            : $"  ПРОВАЛОВ: {bad}");
    }
    ...
    static void LootCheck()
    {
        ...
        foreach (var rule in TitleDatabase.Rules)
        {
            ...
            // Оба условия обязаны связывать. Счёт, набираемый втрое
            // раньше важности, — мёртвое поле: имя держит одна важность,
            // и второе условие можно было бы удалить, не заметив.
            //
            // Меряем отношение: за сколько трупов набирается счёт
            // и за сколько важность. Если они разъехались вдвое —
            // одно из двух ничего не решает.
            double byCount = rule.RequiredCount;
            double ratio = byCount / Math.Max(nowN, 1);
            if (ratio < 0.5 || ratio > 2.0) idleCount++;
        }

        if (idleCount > 0)
            Console.WriteLine($"\n  ПРОВАЛ: у {idleCount} из {lootRules} жадных титулов "
                            + "счёт и важность разъехались больше чем вдвое — "
                            + "одно из двух условий ничего не решает");
